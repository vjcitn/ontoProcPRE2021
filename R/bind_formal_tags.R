#' add mapping from informal to formal cell type tags to a SingleCellExperiment colData
...
#' @return SingleCellExperiment instance with a new colData column 'formal' giving
...
#' @export
bind_formal_tags = function(sce, informal, tagmap, force=FALSE) {
  cdn = names(colData(sce))
  if (!(informal %in% cdn)) stop(sprintf("can't find %s in names(colData(sce))", informal))
  ...
  colData(sce)$formal = mapv[sce[[informal]]]
  sce
}
